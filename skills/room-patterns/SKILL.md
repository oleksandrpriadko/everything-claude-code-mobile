...
// Required for exportSchema = true (below) to actually write schema JSON files —
// without this block KSP just emits a build warning and exports nothing.
room {
...
- Use `@Upsert` for upsert patterns (not `@Insert(onConflict = OnConflictStrategy.REPLACE)` — REPLACE is delete+reinsert under the hood, which resets autogenerated ids and can cascade-delete `ON DELETE CASCADE` foreign-key children); use `@Insert(onConflict = OnConflictStrategy.IGNORE)` for insert-if-absent.
