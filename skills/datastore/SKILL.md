...
**Android/JVM-only version** (uses `java.io.InputStream`/`OutputStream`/`File` — will NOT compile in `commonMain`; use only in `androidMain`/`jvmMain`, or for an Android-only app):
...
**Genuinely KMP-safe version** (`commonMain`, `androidx.datastore:datastore-core-okio`) — use `OkioSerializer`/`OkioStorage` instead of the `java.io`-based `Serializer`/`DataStoreFactory.create(produceFile=...)` pair above:
...
object AppSettingsSerializer : OkioSerializer<AppSettings> {
    ...
    override suspend fun readFrom(source: BufferedSource): AppSettings =
        try { Json.decodeFromString(source.readUtf8()) }
        catch (e: SerializationException) { throw CorruptionException("Cannot read AppSettings", e) }
    override suspend fun writeTo(t: AppSettings, sink: BufferedSink) =
        sink.writeUtf8(Json.encodeToString(t))
}
fun createAppSettingsStore(producePath: () -> String): DataStore<AppSettings> = DataStoreFactory.create(
    storage = OkioStorage(FileSystem.SYSTEM, AppSettingsSerializer) { producePath().toPath() },
    corruptionHandler = ReplaceFileCorruptionHandler { AppSettings() },
)
```
Verify the exact `OkioSerializer`/`OkioStorage` signatures against the `datastore-core-okio` version pinned in the project — this API surface has moved during DataStore's multiplatform migration and may differ slightly across releases.

## KMP factory — per-platform path (not the temp dir)

Applies to both the Preferences and the Okio-based Typed factory above — only the path producer is platform-specific:
