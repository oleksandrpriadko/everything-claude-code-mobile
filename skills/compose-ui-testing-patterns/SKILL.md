...
// Emit hover — interactionSource.emit is a suspend function. Launch it on
// a scope backed by Dispatchers.Unconfined so the emit runs immediately,
// in-line, before `launch` returns — not on a bare `TestScope()`, whose
// default StandardTestDispatcher only *queues* the coroutine and requires
// an explicit advanceUntilIdle()/advanceTimeBy() that nothing here calls.
// Without that, the emit races composeTestRule.waitForIdle() (which only
// drains Compose's own idling resources, not a coroutine scheduler) and
// the assertion below can observe the pre-hover state nondeterministically.
...
- **Emit interactions from a coroutine scope backed by `Dispatchers.Unconfined`**, not a bare `TestScope()`/`StandardTestDispatcher`, since `emit` is a suspend function and an unconfined dispatcher runs the coroutine to its first real suspension point immediately rather than queuing it for a scheduler nothing in the test advances. Do not use `LaunchedEffect` — that is a production Compose effect, not a test tool.
