...
For contexts that want no DI framework at all, wire dependencies by hand with a plain-Kotlin `AppContainer` (constructor wiring, `by lazy` for singletons):
...
On non-Android KMP UI (e.g. Voyager), resolve the equivalent of a ViewModel with `getScreenModel()`:
