...
fun List<Notification>.withDedupIndex(): List<Notification> {
    ...
    return map { item ->
        ...
        // copy() first: if the same Notification instance appears more than
        // once in the source list (aliased reference), mutating dedupIndex
        // in place would overwrite the earlier occurrence's index too,
        // since both entries would point at the same object.
        item.copy().also { it.dedupIndex = count }
    }
}
...
fun SafeShimmerItem(
    ...
    // runCatching around a Canvas(...) call only guards composition of the
    // node; the DrawScope lambda runs later, in the draw phase, so a failure
    // there is never caught and the "fallback" below never fires. Instead,
    // check the measured size up front (as in Section 3) and choose between
    // Canvas and the solid-color fallback before attempting to draw.
    BoxWithConstraints(
        modifier = modifier
            .fillMaxWidth()
            .height(itemHeight)
        ...
        if (maxWidth > 0.dp && maxHeight > 0.dp) {
            ...
            // Fallback: solid color placeholder when size isn't known yet
            Box(
                modifier = Modifier
                    .matchParentSize()
...
**Rule:** Guard shimmer and placeholder drawing with explicit size checks made *before* invoking `Canvas`/`DrawScope`, not with `try/catch`/`runCatching` around the composable call — draw-phase exceptions happen after the composable call returns and are never caught that way. Provide a solid-color fallback so the loading state is never invisible and never crashes. Avoid `runCatching` in suspend contexts regardless — it catches `CancellationException` and breaks structured concurrency.
