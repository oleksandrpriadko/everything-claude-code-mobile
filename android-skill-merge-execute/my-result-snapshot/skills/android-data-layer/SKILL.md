...
3. **`setQueryCoroutineContext(Dispatchers.IO)`** — Android Room defaults this; KMP doesn't. **`Dispatchers.IO` does not resolve in plain `commonMain`** — it's part of `kotlinx-coroutines-core`'s JVM/Native-specific API, unavailable on JS/Wasm. The `getRoomDatabase` builder function below must live in an intermediate concurrent source set (e.g. `concurrentMain`/`jvmNativeMain`) shared only by targets where `Dispatchers.IO` exists, not in plain `commonMain` alongside the `@Database`/`@ConstructedBy` declarations.
...
// concurrentMain (Android/iOS/JVM — wherever Dispatchers.IO resolves; NOT commonMain)
