...
## Test dependency stack (JUnit5 + MockK + Turbine + Kotest)

If the project hasn't already standardized on a stack, this combination covers unit, coroutine-flow, and Compose UI tests:
...
These examples use MockK for brevity; prefer a fake over a mock for collaborators with real behaviour worth modeling (per the fakes-over-mocks default this skill assumes elsewhere) and reserve mocks for verifying interaction/call-count.

## Coverage target

Target line coverage of 80% as a floor, not a goal — a high percentage with weak assertions (see `core:testing-behavior`) is worse than a lower percentage of mutation-resistant tests.
