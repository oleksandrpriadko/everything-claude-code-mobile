...
For the common case of "run X while the screen is STARTED/RESUMED," prefer the purpose-built lifecycle effects from `lifecycle-runtime-compose` 2.8+ over hand-rolling `DisposableEffect` + `LifecycleEventObserver`: `LifecycleStartEffect(key) { onStart(); onStopOrDispose { onStop() } }` for STARTED ⇄ STOPPED with paired cleanup, `LifecycleResumeEffect(key) { onResume(); onPauseOrDispose { onPause() } }` for RESUMED ⇄ PAUSED, and `LifecycleEventEffect(Lifecycle.Event.ON_PAUSE) { ... }` for a single event with no pair semantics. The key argument follows the same rules as `LaunchedEffect`. Reach for the raw `DisposableEffect` + `LifecycleEventObserver` pattern only on lifecycle 2.7 and older, or for an event these three don't expose.
